...
class Player(pygame.sprite.Sprite):
    def __init__(self, grid_size, walkables, maze, spritefolder = "Sprites/assassin/"):
        super().__init__()
        self.maze = maze
        ...
        self.image = pygame.transform.scale_by(self.image, int(self.grid_size/16))
        self.rect = self.image.get_rect()
        
        self.stridetime = 10
        self.animation_state = 1
        self.animation_frame_time = 40
        self.ismoving = False
        self.facing = "front"
    ...
    def setplayerimg(self, s):
        self.facing = s
        self.image = pygame.image.load(self.spritefolder + f"{s}{self.animation_state}.png").convert_alpha()
        self.image = pygame.transform.scale_by(self.image,int(self.grid_size/16))
    ...
    def update(self):
        ...
        for value in self.keymaps.values():
            allfalse = allfalse and (not value[0])
        if allfalse:
            self.ismoving = False
        for key, value in self.keymaps.items():
            if value[0]:
                value[1]+=1
                if(value[1] > self.stridetime):
                    self.ismoving = True

        isany = False
        t = np.array(list(self.keymaps.values()), dtype=bool)[:,0]
        if((t).any()):
            isany = True
            keyval = np.array(list(self.keymaps.keys()), dtype=str)[t][0]

        if self.ismoving and isany:
            self.move(keyval)
